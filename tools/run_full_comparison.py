...
def plot_results(summary, out_png):
    ...
    fig, axes = plt.subplots(
        len(workloads), 3, figsize=(12, 3 * len(workloads))
    )
    if len(workloads) == 1:
        axes = np.array([axes])
    ...
    for i, wl in enumerate(workloads):
        ...
        miss = [
            summary[wl].get(p, {}).get("l2_miss_rate", 0.0) for p in policies
        ]
        base = ipc[0] if ipc and ipc[0] else None
        spd = [
            ((x / base - 1.0) * 100.0) if (base and x) else 0.0 for x in ipc
        ]
        ...
        ax0.set_title(f"{wl} IPC")
        ax0.set_ylabel("IPC")
        if any(ipc):
            ymin, ymax = min(ipc), max(ipc)
...
def main():
    ap = argparse.ArgumentParser(
        description="Run LRU/AIP/LvP across workloads, aggregate and plot."
    )
    ap.add_argument(
        "--workloads", nargs="+", default=["memory_benchmark", "cache_stress"]
    )
    ...
    for wl in args.workloads:
        ...
        for pol in ["LRU", "AIP", "LvP"]:
            ...
            stats = run_gem5(
                args.gem5, wl, pol, args.max_insts, outdir, l2_size="512kB"
            )
        ...
        stats576 = run_gem5(
            args.gem5, wl, "LRU", args.max_insts, outdir576, l2_size="576kB"
        )
    ...
    with open(csv_path, "w") as fh:
        fh.write(
            "workload,policy,ipc,l2_miss_rate,l2_misses,l2_accesses,sim_ticks\n"
        )
