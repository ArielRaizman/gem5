...
def main():
    ap = argparse.ArgumentParser(description='Plot IPC, speedup, and L2 miss rate from summary JSON.')
    ap.add_argument('--in', dest='in_path', default='parsec_results/comparison_summary.json')
    ap.add_argument('--out', dest='out_path', default='parsec_results/comparison_summary_plot.png')
    args = ap.parse_args()
    ...
    labels = ['LRU', 'AIP', 'LvP']
    ipc = [d.get('LRU', {}).get('ipc'), d.get('AIP', {}).get('ipc'), d.get('LvP', {}).get('ipc')]
    base = ipc[0]
    speed = [((x / base - 1.0) * 100.0) if (x and base) else 0.0 for x in ipc]
    miss_rate = [d.get('LRU', {}).get('l2_miss_rate'), d.get('AIP', {}).get('l2_miss_rate'), d.get('LvP', {}).get('l2_miss_rate')]
    ...
    if all(isinstance(x, (int, float)) for x in miss_rate) and None not in miss_rate:
        ymin = min(miss_rate)
        ymax = max(miss_rate)
        span = max(1e-6, ymax - ymin)
