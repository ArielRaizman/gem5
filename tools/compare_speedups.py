    ...
    for p in paths_or_globs:
        if os.path.isdir(p):
            for root, _, fnames in os.walk(p):
                files.extend(
                    [os.path.join(root, f) for f in fnames if f.endswith('.json')]
                )
        else:
            files.extend(glob(p))
    results = []
...
def _index_by_app(entries: List[Dict]) -> Dict[str, Dict]:
    ...
    for e in entries:
        ...
        # Normalize metrics
        metrics = {
            'IPC': e.get('IPC') or e.get('ipc') or e.get('sim_IPC') or e.get('cpu.IPC'),
            'L2_MissRate': e.get('L2_MissRate') or e.get('l2_miss_rate') or e.get('L2.miss_rate'),
...
def merge_sets(lru512_paths: List[str], aip_paths: List[str], lvp_paths: List[str], lru576_paths: List[str]) -> Tuple[Dict[str, Dict], Dict[str, Dict], Dict[str, Dict], Dict[str, Dict]]:
...
def compute_speedups(lru512: Dict[str, Dict], other: Dict[str, Dict]) -> Dict[str, float]:
    out: Dict[str, float] = {}
...
def try_plot_bar(apps: List[str], series: Dict[str, Dict[str, float]], out_path: str) -> None:
    try:
        import matplotlib.pyplot as plt
        import numpy as np
    except Exception as e:
        print(f"warn: plotting skipped (matplotlib not available): {e}", file=sys.stderr)
        return
    ...
    for i, key in enumerate(keys[:3]):
        ...
        bars.append(ax.bar(x + offs[i], values, width, label=key, color=colors[i % len(colors)]))
...
def main():
    ap = argparse.ArgumentParser(description='Compute speedups and storage-overhead proxy, then plot.')
    ap.add_argument('--lru512', nargs='+', required=True, help='JSON files/dirs/globs for LRU 512KB runs')
    ap.add_argument('--aip512', nargs='+', required=False, default=[], help='JSON files/dirs/globs for AIP 512KB runs')
    ap.add_argument('--lvp512', nargs='+', required=False, default=[], help='JSON files/dirs/globs for LvP 512KB runs')
    ap.add_argument('--lru576', nargs='+', required=False, default=[], help='JSON files/dirs/globs for LRU 576KB runs (storage overhead proxy)')
    ap.add_argument('--out-json', default='parsec_results/comparison_summary.json')
    ap.add_argument('--out-csv', default='parsec_results/comparison_summary.csv')
    ap.add_argument('--out-plot', default='parsec_results/comparison_summary.png')
    args = ap.parse_args()

    lru512, aip512, lvp512, lru576 = merge_sets(args.lru512, args.aip512, args.lvp512, args.lru576)
    if not lru512:
    ...
    summary = {
        ...
        'averages': {
            ...
            'LRU576_vs_LRU512_%': average(speed_lru576) if speed_lru576 else None,
        },
        'per_app': {
            app: {
    ...
    rows = [['app', 'baseline_IPC', 'AIP_speedup_%', 'LvP_speedup_%', 'LRU576_speedup_%']]
    for app in apps:
        rows.append([
            app,
